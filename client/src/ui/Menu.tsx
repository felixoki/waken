...
export function Menu({ ready }: { ready: () => void }) {
  ...
  const create = async () => {
    ...
    if (result.error) return fail("Failed to create world");

    if (!result.data.ok) return fail("Failed to create world");

    const dataResult = await tryCatch(result.data.json());

    if (dataResult.error) return fail("Failed to parse response");
  ...
  const load = async () => {
    ...
    if (result.error) return fail("Failed to load worlds");

    if (!result.data.ok) return fail("Failed to load worlds");
  ...
  const start = async (worldId: string) => {
    ...
    if (result.error) return fail("Failed to start world");

    if (!result.data.ok) return fail("Failed to start world");
  ...
  const join = async () => {
    ...
    if (result.error) return fail("Failed to join world");

    if (!result.data.ok) return fail("World not found or failed to join");
      ...
      {starting && (
        <>
          <video
            ...
            src="./assets/videos/mountains_start.mp4"
          ...
          >
            <p
              className={`absolute bottom-8 right-8 text-white/40 text-sm tracking-widest transition-opacity duration-1000 ${
                playing && !played ? "opacity-100" : "opacity-0"
              }`}
            >
              Click to skip
            </p>
          ...
          <div
            className="absolute inset-0 bg-gray-900 pointer-events-none transition-opacity duration-1000"
            style={{ opacity: played ? 1 : 0 }}
            onTransitionEnd={(e) => {
              if (e.propertyName !== "opacity") return;
              if (played) setFaded(true);
            }}
          />
        </>
      )}
      <div
        className={`absolute inset-0 transition-opacity duration-700 ${starting ? "opacity-0 pointer-events-none" : ""}`}
      >
