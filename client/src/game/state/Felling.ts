...
import { DURATION_FELLING, EXTRACTION_IMPACT_FRAME } from "@server/globals";
import { State } from "./State";
import { Entity } from "../Entity";
import { AnimationComponent } from "../components/Animation";
import { vfx } from "../vfx";

export class Felling implements State {
  public name = StateName.FELLING;
  ...
  enter(entity: Entity): void {
    ...
    this.timer = entity.scene.time.delayedCall(DURATION_FELLING, () => {
      this._chop(entity);
      this.exit(entity);
    });
  }
  ...
  private _impact(entity: Entity): void {
    entity.scene.managers.sound.play.sfx(SoundName.CHOP, {
  ...
  private _chop(entity: Entity): void {
    const isLocal = entity === entity.scene.managers.players.player;
